...
class Lista {
    ...
    public void ordenar(int k){
        ...
        Instituicao menor = new Instituicao();
        int pos;
        for(int i=0;i<k;i++){
            menor = this.array[i].clone();
            pos = i;
            for(int j=i+1;j<this.qtd;j++,this.comps++){
                if(this.array[j].getCodigo()<menor.getCodigo()){
                    menor = this.array[j].clone();
                    pos=j;
                }
                if(menor.getCodigo()!=this.array[i].getCodigo()){
                    aux = this.array[i].clone();
                    this.array[i] = menor.clone();
                    this.array[pos] = aux.clone();
                    this.swaps++;
                }
            }
        }
...
public class Ex8{
    ...
    public static void main(String[] args) throws Exception{
        ...
        Arq.openWrite("635382_selecao.txt");
