...
class Lista {
    ...
    public void ordenar(int k){
        ...
        for(i=1;i<this.qtd;i++){
            tmp = this.array[i].clone();
            ini = (i>k-1) ? k-1 : i-1;
            for(j=ini;j>=0 && this.array[j].getCodigoMantenedora()>tmp.getCodigoMantenedora();j--,this.comps++){
                this.array[j+1]=this.array[j].clone();
                this.swaps++;
            }
            this.array[j+1]=tmp.clone();
        }
    }
...
public class Ex9{
    ...
    public static void main(String[] args) throws Exception{
        ...
        Arq.openWrite("635382_insercao.txt");
